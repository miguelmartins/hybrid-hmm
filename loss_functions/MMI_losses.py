...
class MMILoss(Loss):
    ...
    def _complete_likelihood(self, y_true, y_pred):
        ...
        joint = self.p_states[sequence[0]] * emissions[0]
        c_t = 1 / tf.reduce_sum(joint)
        joint = joint * c_t
        C_t = tf.math.log(c_t)
        for t in range(1, T):
            joint = joint * self.trans_mat[sequence[t - 1], sequence[t]] * emissions[t]
            c_t = 1 / tf.reduce_sum(joint)
            joint = joint * c_t
            C_t += tf.math.log(c_t)
        return -C_t
    ...
    def _complete_likelihood_deprecated(self, y_true, y_pred):
        T = tf.shape(y_true)[0]
        sequence = tf.argmax(y_true, axis=1)
        emissions = tf.reduce_sum(tf.math.log(tf.reduce_max(y_true * y_pred, axis=1) + 1e-12))
